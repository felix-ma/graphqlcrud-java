...
public class SQLQueryBuilderVisitor implements QueryVisitor{
    ...
    public void visitArgument(Field field, GraphQLFieldDefinition definition, GraphQLObjectType type, Argument arg) {
        ...
        if(argName.equals("page")) {
        ...
        } else if (argName.equals("input") && field.getName().contains("create")) {
            Condition condition;
            String pkName = null;
            boolean flag = false;
            for (GraphQLFieldDefinition f : type.getFieldDefinitions()) {
                // check for fields of ID type ie primary keys
                if (f.getType().equals(GraphQLNonNull.nonNull(Scalars.GraphQLID))) {
                    pkName = f.getName();
                    for (ObjectField objectField : ((ObjectValue) argValue).getObjectFields()) {
                        if (f.getName().equals(objectField.getName())) {
                            flag = true;
                            String name = objectField.getName();
                            Value<?> value = objectField.getValue();
                            condition = filterBuilder.buildCondition(name, "eq", value);
                            vctx.condition = vctx.condition == null ? condition : filterBuilder.and(vctx.condition, condition);
                        }
                    }
                }
            }
            if(!flag) {
                ...
                Object keyValue = ctx.getAutoGeneratedPrimaryKey();
                if(keyValue != null) {
                    condition = filterBuilder.buildCondition(pkName, "eq", (Value<?>) keyValue);
                    vctx.condition = vctx.condition == null ? condition : filterBuilder.and(vctx.condition, condition);
                } else {
                    throw new RuntimeException("No ID value is provided");
                }
            }
        }
        else {
